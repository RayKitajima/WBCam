...
@interface PreviewHelper_iPod4_iOS5(Private)
...
@implementation PreviewHelper_iPod4_iOS5
...
{
    // **************************************************
	// prepare iPod4 iOS5 specific perview layer object
    // **************************************************
	self.layer = [PreviewLayer_iPod4_iOS5 layer];
	...
	unsigned char *reducerAddress = (unsigned char *)malloc(IPOD4_IOS5_PREVIEW_REDUCER_SIZE);
	reducer = reducerAddress;
	
	pixelNumbers = (unsigned char *)malloc(IPOD4_IOS5_PREVIEW_REDUCED_PIXELS*4);
	
	unsigned char *pixel_cursor = pixelNumbers;
	int *pixel_cursor_int;
	int W = IPOD4_IOS5_PREVIEW_BUFFER_HEIGHT; // reducedWidth
	int H = IPOD4_IOS5_PREVIEW_BUFFER_WIDTH;  // reducedHieght 
    int p = IPOD4_IOS5_PREVIEW_BUFFER_PADDING;
    int Hp = H + p;
	int n = 0;
	int n2 = 0;
    for( int j=0; j<H; j+=IPOD4_IOS5_PREVIEW_REDUCE_RATE ){
        for( int i=0; i<W; i+=IPOD4_IOS5_PREVIEW_REDUCE_RATE ){
            n = (Hp * W) - (Hp * i) - j - 1;
	...
	sortedPixelNumbers = (unsigned char *)malloc(IPOD4_IOS5_PREVIEW_REDUCED_PIXELS*4);
	ArrangePixelNumbersForNeon((int *)pixelNumbers,(int *)sortedPixelNumbers,IPOD4_IOS5_PREVIEW_REDUCED_PIXELS);
}
...
- (void) setWhiteBalanceParam:(float)Tx Ty:(float)Ty
{
    ...
    float Bx = (float)IPOD4_IOS5_SCREEN_WIDTH * Tx;
	float By = (float)IPOD4_IOS5_SCREEN_HEIGHT * Ty;
	
	NSLog(@"Touch Point         : %1.3f, %1.3f", Tx, Ty);
	NSLog(@"Bitmap Point        : %1.3f, %1.3f", Bx, By);
	
    int pixelNumber = ( IPOD4_IOS5_PREVIEW_REDUCED_HEIGHT - (int)By ) * IPOD4_IOS5_PREVIEW_REDUCED_WIDTH + (int)Bx;
    ...
    int sGw = 0;
    int sBw = 0;
    for( int i = -1 * IPOD4_IOS5_POINT_SPREAD_SIZE; i < IPOD4_IOS5_POINT_SPREAD_SIZE; i++ ){
        for( int j = -1 * IPOD4_IOS5_POINT_SPREAD_SIZE; j < IPOD4_IOS5_POINT_SPREAD_SIZE; j++ ){
            int pixelnum = pixelNumber - i * IPOD4_IOS5_PREVIEW_REDUCED_WIDTH + j;
            if( pixelnum < 0 ){ continue; }
            if( pixelnum > IPOD4_IOS5_PREVIEW_REDUCED_PIXELS ){ continue; }
...
static inline CGImageRef CreateCGImageFromPixelBuffer_Impl(WhiteBalanceProcessorDef *whiteBalanceProcessor, CVPixelBufferRef pixelBuffer, unsigned char *reducer, unsigned char *pixelNumbers, int *wbp)
{
	...
	//
	[whiteBalanceProcessor processBitmapBGRA:baseAddress reducer:reducer pixelNumber:(int *)pixelNumbers whitebalanceParameter:wbp reducedPixelNumber:IPOD4_IOS5_PREVIEW_REDUCED_PIXELS*4];
	
    //	NSDate *then = [NSDate date];
    //	NSLog(@"delay [wbp]: %1.3fsec", [then timeIntervalSinceDate:now]);
	
	CGImageRef cgimage = CreateCGImageFromBitmap(reducer,IPOD4_IOS5_PREVIEW_REDUCED_WIDTH,IPOD4_IOS5_PREVIEW_REDUCED_HEIGHT);
