...
@implementation DeviceConfig_iPod4_iOS5

- (id)init
{
    self = [super init];
    
    applicationMode = @"ipod4,* iOS5";
    deviceIdentification = kDeviceIdentification_iPod4_iOS5;
    ...
    previewBufferWidth  = 360;
    previewBufferHeight = 480;
    ...
    previewWidthAdjusted  = (int)( ratio * (float)previewWidth );  // 360
    previewHeightAdjusted = (int)( ratio * (float)previewHeight ); // 480
    previewRectAdjusted   = CGRectMake(0, 0, (float)previewWidthAdjusted, (float)previewHeightAdjusted);
    
    // calculate adjustment
    
    previewScreenAdjustmentX = (int)( abs(previewWidthAdjusted - previewWidth) / 2.0f );   // (360 - 320)/2 = 20 : hardcoded in Previewlayer
    previewScreenAdjustmentY = (int)( abs(previewHeightAdjusted - previewHeight) / 2.0f ); // (480 - 427)/2 = 26 : hardcoded in Previewlayer
    
    // stillimage also 960x720
    stillimageWidth  = 720;
    stillimageHeight = 960;
    stillimageSize   = (960*720*4);
    stillimagePixels = (960*720);
    
    // album preview image reduce rate
    albumPreviewImageReduceRate = 2; // 1/2
    ...
    dummyBlackImage = [UIImage imageNamed:@"dummy_black_320x426.png"];
    albumBgImage    = [UIImage imageNamed:@"album_bg_320x480.png"];
