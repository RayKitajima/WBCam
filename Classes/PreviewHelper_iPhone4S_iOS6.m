...
#import "PreviewLayer_iPhone4S_iOS6.h"
#import "DeviceConfig_iPhone4S_iOS6.h"

@interface PreviewHelper_iPhone4S_iOS6(Private)
...
@implementation PreviewHelper_iPhone4S_iOS6
...
{
    // ***************************************************
	// prepare iPhone4S iOS6 specific perview layer object
    // ***************************************************
	self.layer = [PreviewLayer_iPhone4S_iOS6 layer];
	...
	unsigned char *reducerAddress = (unsigned char *)malloc(IPHONE4S_iOS6_PREVIEW_REDUCER_SIZE);
	reducer = reducerAddress;
	
	pixelNumbers = (unsigned char *)malloc(IPHONE4S_iOS6_PREVIEW_REDUCED_PIXELS*4);
	
	unsigned char *pixel_cursor = pixelNumbers;
	int *pixel_cursor_int;
	int W = IPHONE4S_iOS6_PREVIEW_BUFFER_HEIGHT; // reducedWidth
	int H = IPHONE4S_iOS6_PREVIEW_BUFFER_WIDTH;  // reducedHieght
    int p = IPHONE4S_iOS6_PREVIEW_BUFFER_PADDING;
    int Hp = H + p;
	int n = 0;
	int n2 = 0;
    for( int j=0; j<H; j+=IPHONE4S_iOS6_PREVIEW_REDUCE_RATE ){
        for( int i=0; i<W; i+=IPHONE4S_iOS6_PREVIEW_REDUCE_RATE ){
            n = (Hp * W) - (Hp * i) - j - 1;
	...
	sortedPixelNumbers = (unsigned char *)malloc(IPHONE4S_iOS6_PREVIEW_REDUCED_PIXELS*4);
	ArrangePixelNumbersForNeon((int *)pixelNumbers,(int *)sortedPixelNumbers,IPHONE4S_iOS6_PREVIEW_REDUCED_PIXELS);
}
...
- (void) setWhiteBalanceParam:(float)Tx Ty:(float)Ty
{
    ...
    float Bx = (float)IPHONE4S_iOS6_SCREEN_WIDTH * Tx;
	float By = (float)IPHONE4S_iOS6_SCREEN_HEIGHT * Ty;
	
	NSLog(@"Touch Point         : %1.3f, %1.3f", Tx, Ty);
	NSLog(@"Bitmap Point        : %1.3f, %1.3f", Bx, By);
	
    int pixelNumber = ( IPHONE4S_iOS6_PREVIEW_REDUCED_HEIGHT - (int)By ) * IPHONE4S_iOS6_PREVIEW_REDUCED_WIDTH + (int)Bx;
    ...
    int sGw = 0;
    int sBw = 0;
    for( int i = -1 * IPHONE4S_IOS6_POINT_SPREAD_SIZE; i < IPHONE4S_IOS6_POINT_SPREAD_SIZE; i++ ){
        for( int j = -1 * IPHONE4S_IOS6_POINT_SPREAD_SIZE; j < IPHONE4S_IOS6_POINT_SPREAD_SIZE; j++ ){
            int pixelnum = pixelNumber - i * IPHONE4S_iOS6_PREVIEW_REDUCED_WIDTH + j;
            if( pixelnum < 0 ){ continue; }
            if( pixelnum > IPHONE4S_iOS6_PREVIEW_REDUCED_PIXELS ){ continue; }
...
static inline CGImageRef CreateCGImageFromPixelBuffer_Impl(WhiteBalanceProcessorDef *whiteBalanceProcessor, CVPixelBufferRef pixelBuffer, unsigned char *reducer, unsigned char *pixelNumbers, int *wbp)
{
	...
	//
	[whiteBalanceProcessor processBitmapBGRA:baseAddress reducer:reducer pixelNumber:(int *)pixelNumbers whitebalanceParameter:wbp reducedPixelNumber:IPHONE4S_iOS6_PREVIEW_REDUCED_PIXELS*4];
	
    //	NSDate *then = [NSDate date];
    //	NSLog(@"delay [wbp]: %1.3fsec", [then timeIntervalSinceDate:now]);
	
	CGImageRef cgimage = CreateCGImageFromBitmap(reducer,IPHONE4S_iOS6_PREVIEW_REDUCED_WIDTH,IPHONE4S_iOS6_PREVIEW_REDUCED_HEIGHT);
